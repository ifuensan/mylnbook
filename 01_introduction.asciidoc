...
Nodo:: ((("nodo, definido")))Una computadora que participa en una red. Un nodo de Lightning es una computadora que participa en la Red Lightning. Un nodo de Bitcoin es una computadora que participa en la red de Bitcoin. Por lo general, un usuario de LN ejecutará un nodo de Lightning _y_ un nodo de Bitcoin.

On-chain versus off-chain:: ((("pago on-chain","definido")))Un pago está _en la cadena_ si se registra como una transacción en la cadena de bloques de Bitcoin (u otra cadena subyacente). ((("off-chain payment")))Los pagos enviados a través de canales de pago entre nodos de Lightning, y que no son visibles en la cadena de bloques subyacente, se llaman pagos _fuera de la cadena_. Por lo general, en la Red Lightning, las únicas transacciones en la cadena son aquellas utilizadas para abrir y cerrar un canal de pago de Lightning. Existe un tercer tipo de transacción de modificación de canal, llamada "splicing", que se puede utilizar para agregar/eliminar la cantidad de fondos comprometidos en un canal.
